...
- CSS3 with modern and latest wide compatible features. Tailwind CSS is ONLY allowed as per Tailwind CSS Usage Policy established below
- HTML5 semantic markup generated by the aem.live backend, decorated by our code
- Node.js tooling

### Tailwind CSS Usage Policy
By default, use vanilla CSS3 for all styling. Use Tailwind CSS **ONLY** if:
1. A Custom Agent instructions or configuration explicitly mention to use Tailwind CSS.
2. A prompt specifically requests to use Tailwind CSS for a task.
3. An existing file, block, or component already uses Tailwind CSS (maintain consistency).
