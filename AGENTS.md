...
- CSS3 with modern features (preferred). Tailwind CSS is allowed ONLY when explicitly specified by a Custom Agent configuration
- HTML5 semantic markup generated by the aem.live backend, decorated by our code
- Node.js tooling

### Tailwind CSS Usage Policy
By default, use vanilla CSS3 for all styling. Tailwind CSS may be used **only** when:
1. A Custom Agent explicitly enables it in its configuration
2. The specific block or component use utility-first styling for rapid prototyping
